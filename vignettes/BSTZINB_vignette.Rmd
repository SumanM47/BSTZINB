...
m<-apply(A,1,sum)
tpop = USAcities %>% filter(state_id=="IA") %>% group_by(county_fips) %>% summarise(tpop=population) %>% .[,2] %>% unlist %>% as.numeric # log population of counties
```
...
eta1<-Xtilde%*%true.alpha+offset+Phi1+Phi2*t
...
eta2<-Xtilde[u==1,]%*%true.beta+offset[u==1]+Phi3[u==1]+Phi4[u==1]*t[u==1] # Linear predictor for count part
...
this_dat <- data.frame(sid,tid,offset,y,X)
...
```{r}
Xtilde = cbind(offset,X)

res0 = BNB(y, Xtilde, A, oind=1, nchain=3, niter=100, nburn=50)
glimpse(res0)

res1 = BZINB(y, Xtilde, A, oind=1, nchain=3, niter=100, nburn=50)
glimpse(res1)
...
```{r}
res2 = BSTNB(y, Xtilde, A, oind=1, nchain=3, niter=100, nburn=50)
glimpse(res2)

apply(res2$Beta,2,mean)
```
...
res3 = BSTZINB(y, Xtilde, A, oind=1, LinearT=TRUE, nchain=3, niter=100, nburn=50, nthin=1)
glimpse(res3)
...
par(mfrow=c(1,3));plot(res3$R[,1],type='o',main="R");plot(res3$R[,2],type='o',main="R");plot(res3$R[,3],type='o',main="R")
...
```{r,fig.width=6,fig.height=3,fig.align='center'}
res4 = BSTZINB(y, Xtilde, A, oind=1, LinearT=FALSE, nchain=3, niter=100, nburn=50)
glimpse(res4)
...
par(mfrow=c(1,3));plot(res4$R[,1],type='o',main="R");plot(res4$R[,2],type='o',main="R");plot(res4$R[,3],type='o',main="R")
...
```{r}
ResultTableSummary2(y, Xtilde, A, oind=1, LinearT=FALSE, nchain=3, niter=100, nburn=20, nthin=1)
```
