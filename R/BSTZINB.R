...
BSTZINB <- function(y, X, A, LinearT = TRUE, nchain = 3, niter = 100, nburn = 20, nthin = 1){
  ...
  for(chain in 1:nchain){
    ...
    beta <- alpha <- rnorm(p)
    phi_init <- spam::rmvnorm(1, sigma = diag(0.1, 16*n))	  # Random effects
    ...
    for (i in 1:niter){
      ...
      # Update alpha
      mu <- Xtilde%*%alpha + Phi1 + Phi2*tss
      w <- BayesLogit::rpg(N, 1, mu)
      z <- (y1 - 0.5)/w
      v <- solve(crossprod(sqrt(w)*Xtilde) + T0a)
      m <- v%*%(T0a%*%alpha0 + t(sqrt(w)*Xtilde)%*%(sqrt(w)*(z - Phi1 - Phi2*tss)))
      ...
      m <- c(priorprec%*%priormean) + tapply(w*(z - Xtilde%*%alpha - Phi2*tss), sid, "sum")
      if(matrixcalc::is.positive.definite(prec%>%as.matrix)) phi1 <- spam::rmvnorm.canonical(1, m, prec)[1,]
      ...
      prec <- priorprec + spam::as.spam(diag(tapply(w*tss^2, sid, "sum"), n, n))
      m <- c(priorprec%*%priormean) + tapply(tss*w*(z - Xtilde%*%alpha - Phi1), sid, "sum")
      if(matrixcalc::is.positive.definite(prec%>%as.matrix)) phi2 <- spam::rmvnorm.canonical(1, m, prec)[1,]
      ...
      eta1 <- Xtilde%*%alpha + Phi1 + Phi2*tss
      eta2 <- Xtilde%*%beta + Phi3 + Phi4*tss              # Use all n observations
      ...
      # Update beta
      eta <- Xtilde[y1 == 1,]%*%beta + Phi3[y1 == 1] + Phi4[y1 == 1]*tss[y1 == 1]
      w <- BayesLogit::rpg(N1, y[y1 == 1]+ r, eta)                               # Polya weights
      z <- (y[y1 == 1] - r)/(2*w)
      v <- solve(crossprod(Xtilde[y1 == 1,]*sqrt(w)) + T0b)
      m <- v%*%(T0b%*%beta0+t(sqrt(w)*Xtilde[y1 == 1,])%*%(sqrt(w)*(z - Phi3[y1 == 1] - Phi4[y1 == 1]*tss[y1 == 1])))
      ...
      prec <- priorprec + spam::as.spam(diag(tapply(w, sid[y1 == 1],"sum"), n, n))
      tmp <- rep(0, n)                       # Account empty blocks
      tmp[nis1 > 0] <- tapply(w*(z - Xtilde[y1 == 1,]%*%beta - Phi4[y1 == 1]*tss[y1 == 1]), sid[y1 == 1],"sum")
      ...
      prec <- priorprec + spam::as.spam(diag(tapply(w*tss[y1 == 1]^2, sid[y1 == 1],"sum"), n, n))
      tmp <- rep(0, n)                       # Account for empty counties
      tmp[nis1 > 0] <- tapply(tss[y1 == 1]*w*(z - Xtilde[y1 == 1,]%*%beta - Phi3[y1 == 1]), sid[y1 == 1], "sum")
      m <- c(priorprec%*%priormean) + tmp
      ...
      phimat <- cbind(phi1, phi2, phi3, phi4)
      try({
        # Sigmaphi <- MCMCpack::riwish(4 + n - 1, diag(4) + t(phimat)%*%Q%*%phimat)
        Sigmaphi <- solve(stats::rWishart(1, 4 + n - 1, solve(diag(4) + crossprod(phimat,Q%*%phimat)))[,,1])
      })
