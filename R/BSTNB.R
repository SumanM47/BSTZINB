...
BSTNB <- function(y, X, A, nchain = 3, niter = 100, nburn = 20, nthin = 1){
  ...
  for(chain in 1:nchain){
    ...
    #########
    beta <- rnorm(p)
    phi_init <- spam::rmvnorm(1, sigma = diag(0.1, 2*n))	  # Random effects
    ...
    for (i in 1:niter){
     ...
     # Update beta
      eta <- X%*%beta + Phi3 + Phi4*tss
      w <- BayesLogit::rpg(N, y + r, eta)                               # Polya weights
      z <- (y - r)/(2*w)
      v <- solve(crossprod(X*sqrt(w)) + T0b)
      m <- v%*%(T0b%*%beta0 + t(sqrt(w)*X)%*%(sqrt(w)*(z - Phi3 - Phi4*tss)))
      ...
      tmp <- tapply(w*(z - X%*%beta - Phi4*tss), sid, "sum")
      ...
      tmp <- tapply(tss*w*(z - X%*%beta - Phi3), sid, "sum")
      m <- c(priorprec%*%priormean) + tmp
      ...
      phimat <- cbind(phi3, phi4)
      try({
        Sigmaphi <- solve(stats::rWishart(1, 2 + n - 1, solve(diag(2) + crossprod(phimat,Q%*%phimat)))[,,1])
      })
